...
- 실시간화(사용자 요청 "실시간 번역", 2026-09-02): `AgySessionBackend` — `agy -p "" --input-format stream-json --output-format stream-json` 프로세스 1개를 서버 기동 시 warm-up(통역 지시 1턴)하고 유지. 입력은 `{"event":"user","message":{"role":"user","content":…}}` 한 줄, 출력은 `step_update`(agent_response, text_delta) → `result`(response). 실측 문장당 1.4~2.0초(원샷 7초). 턴 타임아웃 30초·프로세스 사망·200턴 초과 시 재기동. 요청은 lock으로 직렬화. 원샷 방식은 `agy-oneshot`으로 유지. 픽스처 `fixtures/agy_stream.jsonl`.
- 2026-09-05 정리·UI 재설계(사용자 요청 "translator-test 삭제, 자동 번역기만 남김, apple-design 스킬로 UI를 쉽게"): gemma-arena 문서를 지우고 앱을 저장소 루트로 올린 뒤, 저장소를 `/Users/cokoroad/voice-translator` 독립 저장소(브랜치 main, 이력 유지)로 옮김. 옛 `translator-test` 폴더는 휴지통으로. UI: 방향 드롭다운·⇄ 대신 **언어별 말하기 버튼 2개**(왼쪽 내 언어, 오른쪽 상대 언어 — 누른 언어로 듣고 반대쪽으로 번역). 상대가 말한 뒤 방향을 안 뒤집어 생기던 오번역("어떻게 오셨나요?" 질문의 2번 경우)을 구조적으로 없앤다. 언어 선택·자동 읽어주기는 ⚙︎ 하단 시트로 한 단계 안에(임계 감쇠 스프링으로 열고 닫음, 드래그·스크림 탭·완료·Esc 로 닫힘, 관성 투영으로 닫힘 여부 결정). 번역문이 화면에서 가장 큰 글자(22px, 자간 -0.01em), 원문은 작게, 카드 탭으로 다시 읽기, 화자별 좌/우 정렬. 헤더·바는 반투명(backdrop-filter), prefers-reduced-motion / reduced-transparency / contrast 대응, 눌림 즉시 반응(:active). 항상 ko→tl·자동 읽기 켜짐은 유지. 통역 지시(SESSION_INSTRUCTION)는 바꾸지 않음.
